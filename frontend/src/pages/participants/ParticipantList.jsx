...
function ParticipantList() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editData) {
                await api.put(`/participants/${editData.id}`, form);
            } else {
    ...
    return (
        <div style={styles.container}>
            <div style={styles.header}>
                <h1 style={styles.title}>👤 Manajemen Participant</h1>
                {user.role === 'admin' && (
                    <button onClick={() => { setShowForm(true); setEditData(null); setForm({ name: '', email: '', phone: '' }); }} style={styles.btnAdd}>
                        + Tambah Participant
                    </button>
                )}
            </div>
            ...
            {showForm && (
                <div style={styles.formCard}>
                    <h2 style={styles.formTitle}>{editData ? 'Edit Participant' : 'Tambah Participant'}</h2>
                ...
                <table style={styles.table}>
                    ...
                    <tbody>
                        {participants.length === 0 ? (
                            <tr><td colSpan="5" style={styles.empty}>Belum ada data participant</td></tr>
                            ...
                            <tr key={p.id} style={i % 2 === 0 ? styles.trEven : styles.trOdd}>
                                ...
                                {user.role === 'admin' && (
                                    <td style={styles.td}>
                                        ...
                                        <button onClick={() => handleDelete(p.id)} style={styles.btnDelete}>Hapus</button>
                                    </td>
                                )}
                            </tr>
                        ))}
